...
int jailbreak(void *init) {
    ...
    write_20(ucred + KSTRUCT_OFFSET_PROC_UID, (void*)buffer);
    uint64_t new_uid = read_64(ucred + KSTRUCT_OFFSET_UCRED_CR_UID);

    uint32_t uid = getuid();

    uint64_t cr_label_pac = read_64(ucred + KSTRUCT_OFFSET_UCRED_CR_LABEL);
    ...
    /* Sandbox patches */
    printf("Sandbox-Slot:\t0x%llx", (cr_label + KSTRUCT_OFFSET_SANDBOX_SLOT));
    write_20(cr_label + KSTRUCT_OFFSET_SANDBOX_SLOT, (void*)buffer);
    printf("\t--->\t0x%llx", read_64(cr_label + KSTRUCT_OFFSET_SANDBOX_SLOT));
    if(check_sandbox_escape() == true) printf("\t\t(success)\n");
    else printf("\t\t(failed)\n");
    
    /* Root User patches */
    printf("Root-User:\t\t\t\t0x%llx\t\t--->\t0x%llx\t\t\t(%s)\n", old_uid, new_uid, uid == 0 ? "success" : "failed");
