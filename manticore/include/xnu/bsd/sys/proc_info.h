...
#define PROC_FLAG_TRACED                                                       \
    2                      /* process currently being traced, possibly by gdb  \
                            */
...
#define PROC_FLAG_PC_THROTTLE                                                  \
    0x200 /* In resource starvation situations, this process is to be          \
           * throttled                                                         \
           */
#define PROC_FLAG_PC_SUSP                                                      \
    0x400 /* In resource starvation situations, this process is to be          \
           * suspended                                                         \
           */
#define PROC_FLAG_PC_KILL                                                      \
    0x600 /* In resource starvation situations, this process is to be          \
           * terminated                                                        \
           */
#define PROC_FLAG_PC_MASK 0x600
/* process action bits for resource starvation */
#define PROC_FLAG_PA_THROTTLE                                                  \
    0x800 /* The process is currently throttled due to resource starvation */
#define PROC_FLAG_PA_SUSP                                                      \
    0x1000 /* The process is currently suspended due to resource starvation */
#define PROC_FLAG_PSUGID                                                       \
    0x2000                    /* process has set privileges since last exec    \
                               */
...
#define PROC_FLAG_DELAYIDLESLEEP                                               \
    0x40000 /* Process is marked to delay idle sleep on disk IO */
#define PROC_FLAG_IOS_IMPPROMOTION                                             \
    0x80000 /* Process is daemon which receives importane donation  */
...
#define TH_STATE_UNINTERRUPTIBLE                                               \
    4 /* thread is in an uninterruptible                                       \
       *  wait */
#define TH_STATE_HALTED                                                        \
    5 /* thread is halted at a                                                 \
       *  clean point */
...
#define SOI_S_DRAINING                                                         \
    0x4000 /* close waiting for blocked system calls to drain */
...
#define PROC_PIDEXITREASONBASICINFOSIZE                                        \
...
#define PROC_FGHW_LEADER_NONUI                                                 \
    11 /* coalition leader is in a non-focal state                             \
        */
#define PROC_FGHW_LEADER_BACKGROUND                                            \
    12 /* coalition leader is in a background state */
#define PROC_FGHW_DAEMON_NO_VOUCHER                                            \
    13 /* pid is a daemon with no adopted voucher */
#define PROC_FGHW_NO_VOUCHER_ATTR                                              \
    14 /* pid has adopted a voucher with no bank/originator attribute */
#define PROC_FGHW_NO_ORIGINATOR                                                \
    15 /* pid has adopted a voucher for a process that's gone away */
#define PROC_FGHW_ORIGINATOR_BACKGROUND                                        \
    16 /* pid has adopted a voucher for an app that's in the background */
...
extern int pid_dynamickqueue_extinfo(proc_t p, kqueue_id_t kq_id,
                                     user_addr_t ubuf, uint32_t bufsize,
                                     int32_t *retval);
