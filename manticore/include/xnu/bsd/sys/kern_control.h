 ...
 */
#define CTLIOCGCOUNT                                                           \
    _IOR('N', 2, int) /* get number of control structures registered */
 ...
 *               unique identifier assigned to the controller. The identifier
 * can be assigned by the system at registration time or be a 32-bit creator
 * code obtained from Apple Computer. sc_unit is a unit number for this sc_id,
 * and is privately used by the kernel controller to identify several instances
 * of the controller.
 ...
 *               or removing the kernel control. This reference will be passed
 * to callbacks for that kernel control.
 */
 ...
 *               connecting to.
 *       @param sac The address used to connect to this control. The field
 * sc_unit contains the unit number of the kernel control instance the client is
 *               connecting to. If CTL_FLAG_REG_ID_UNIT was set when the kernel
 * control was registered, sc_unit is the ctl_unit of the kern_ctl_reg
 * structure. If CTL_FLAG_REG_ID_UNIT was not set when the kernel control was
 *               registered, sc_unit is the dynamically allocated unit number of
 *               the new kernel control instance that is used for this
 * connection.
 *       @param unitinfo A placeholder for a pointer to the optional
 * user-defined private data associated with this kernel control instance.  This
 ...
 *       @param kctlref The control ref for the kernel control instance the
 * client has disconnected from.
 *       @param unit The unit number of the kernel control instance the client
 * has disconnected from.
 *       @param unitinfo The user-defined private data initialized by the
 *               ctl_connect_func callback.
 */
typedef errno_t (*ctl_disconnect_func)(kern_ctl_ref kctlref, u_int32_t unit,
                                       void *unitinfo);
 ...
 */
typedef errno_t (*ctl_send_func)(kern_ctl_ref kctlref, u_int32_t unit,
 ...
 */
typedef errno_t (*ctl_setopt_func)(kern_ctl_ref kctlref, u_int32_t unit,
 ...
 *               is allocated for storage and passed to your function. The
 * length of that buffer is also passed. Upon return, you should set *len to
 * length of the buffer used. In some cases, data may be NULL. When this
 * happens, *len should be set to the length you would have returned had data
 * not been NULL. If the buffer is too small, return an error.
 *       @param kctlref The control ref of the kernel control.
 *       @param unit The unit number of the kernel control instance.
 *       @param unitinfo The user-defined private data initialized by the
 *               ctl_connect_func callback.
 ...
 */
typedef errno_t (*ctl_getopt_func)(kern_ctl_ref kctlref, u_int32_t unit,
                                   void *unitinfo, int opt, void *data,
                                   size_t *len);
 ...
 *               the kernel control socket. The kernel control can use this
 * callback in combination with ctl_getenqueuespace() to avoid overflowing the
 * socket's receive buffer. When ctl_getenqueuespace() returns 0 or
 * ctl_enqueuedata()/ctl_enqueuembuf() return ENOBUFS, the kernel control can
 * wait until this callback is called before trying to enqueue the data again.
 *       @param kctlref The control ref of the kernel control.
 *       @param unit The unit number of the kernel control instance.
 *       @param unitinfo The user-defined private data initialized by the
 *               ctl_connect_func callback.
 ...
 */
typedef void (*ctl_rcvd_func)(kern_ctl_ref kctlref, u_int32_t unit,
                              void *unitinfo, int flags);
 ...
 *               send/sendto/sendmsg (MSG_OOB/MSG_DONTROUTE).
 */
typedef errno_t (*ctl_send_list_func)(kern_ctl_ref kctlref, u_int32_t unit,
                                      void *unitinfo, mbuf_t m, int flags);

/*!
 *       @typedef ctl_bind_func
 *       @discussion The ctl_bind_func is an optional function that allows the
 * client to set up their unitinfo prior to connecting.
 *       @param kctlref The control ref for the kernel control the client is
 *               binding to.
 *       @param sac The address used to connect to this control. The field
 * sc_unit contains the unit number of the kernel control instance the client is
 *               binding to. If CTL_FLAG_REG_ID_UNIT was set when the kernel
 * control was registered, sc_unit is the ctl_unit of the kern_ctl_reg
 * structure. If CTL_FLAG_REG_ID_UNIT was not set when the kernel control was
 *               registered, sc_unit is the dynamically allocated unit number of
 *               the new kernel control instance that is used for this
 ...
 *               kernel control instance.
 */
typedef errno_t (*ctl_bind_func)(kern_ctl_ref kctlref, struct sockaddr_ctl *sac,
                                 void **unitinfo);

/*!
 *       @typedef ctl_setup_func
 *       @discussion The ctl_setup_func is an optional function that allows the
 * client to pick a unit number in the case that the caller hasn't specified one
 *       @param unit A placeholder for a pointer to the unit number that is
 * selected with this kernel control instance
 *       @param unitinfo A placeholder for a pointer to the optional
 * user-defined private data associated with this kernel control instance.  This
 *               opaque info will be provided to the user when the rest of the
 *               callback routines are executed.  For example, it can be used
 ...
 *       @field ctl_name A Bundle ID string of up to MAX_KCTL_NAME bytes
 * (including the ending zero). This string should not be empty.
 *       @field ctl_id The control ID may be dynamically assigned or it can be a
 *               32-bit creator code assigned by DTS.
 *               For a DTS assigned creator code the CTL_FLAG_REG_ID_UNIT flag
 * must be set. For a dynamically assigned control ID, do not set the
 * CTL_FLAG_REG_ID_UNIT flag. The  value of the dynamically assigned control ID
 * is set to this field when the registration succeeds.
 ...
 *               zero, the default receive size will be used, and this default
 * value is set to this field to be retrieved by the caller.
...
struct kern_ctl_reg {
    ...
    ctl_send_list_func
        ctl_send_list; /* Only valid if CTL_FLAG_REG_EXTENDED is set */
 ...
 *               attached. The ctl_connect callback must be specified, the other
 * callbacks are optional. If ctl_connect is set to zero, ctl_register fails
 * with the error code EINVAL.
 *       @param kctlref Upon successful return, the kctlref will contain a
 *               reference to the attached kernel control. This reference is
 * used to unregister the kernel control. This reference will also be passed in
 * to the callbacks each time they are called.
 ...
 *       @param flags Send flags. CTL_DATA_NOWAKEUP and CTL_DATA_EOR are
 * currently the only supported flags.
 ...
 */
errno_t ctl_enqueuedata(kern_ctl_ref kctlref, u_int32_t unit, void *data,
 ...
 */
errno_t ctl_enqueuembuf(kern_ctl_ref kctlref, u_int32_t unit, mbuf_t m,
                        u_int32_t flags);
 ...
 *               ENOBUFS - The queue is full.
 */
errno_t ctl_enqueuembuf_list(kern_ctl_ref kctlref, u_int32_t unit,
                             mbuf_t m_list, u_int32_t flags, mbuf_t *m_remain);
 ...
 *               EINVAL - Invalid parameters.
 */
errno_t ctl_getenqueuepacketcount(kern_ctl_ref kctlref, u_int32_t unit,
                                  u_int32_t *pcnt);
 ...
 *       @discussion Retrieve the amount of space currently available for data
 * to be sent from the kernel control to the client.
 *       @param kctlref The control reference of the kernel control.
 *       @param unit The unit number of the kernel control instance.
 ...
 *               EINVAL - Invalid parameters.
 */
errno_t ctl_getenqueuespace(kern_ctl_ref kctlref, u_int32_t unit,
                            size_t *space);
 ...
 *  EINVAL - Invalid parameters.
 */
errno_t ctl_getenqueuereadable(kern_ctl_ref kctlref, u_int32_t unit,
