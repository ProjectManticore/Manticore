...
void start_rootfs_remount(void){
    ...
    if(runCommandv(argv[0], 3, argv, ^(pid_t pid) {
        kptr_t procStructAddr = look_for_proc(get_path_for_pid(pid));
        if(KERN_POINTER_INVALID(procStructAddr)) printf("Unable to find mount_apfs's process in kernel memory. (%d)\n", pid);
        give_creds_to_proc_at_addr(procStructAddr, get_kernel_cred_addr(g_exp.kernel_proc));
    }, true) != ERR_SUCCESS) printf("-> Unable to mount RootFS.\n");
