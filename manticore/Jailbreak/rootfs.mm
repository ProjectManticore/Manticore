...
void restore_rootfs(void) {
    ...
    if (rootfd <= 0)
        printf("-> Unable to open rootfs!\n");
    const char **snapshots = snapshot_list(rootfd);
    if (snapshots == NULL)
        printf("-> Unable to get snapshot for RootFS.\n");
    if (*snapshots == NULL)
        printf("-> Found no snapshot for RootFS.\n");
    char *snapshot = strdup(*snapshots);
    printf("-> %s\n", snapshot);
    char *systemSnapshot = copySystemSnapshot();
    if (systemSnapshot == NULL)
        printf("-> Unable to copy system snapshot.\n");
    if (execute_with_kernel_credentials(^{
          if (fs_snapshot_rename(rootfd, snapshot, systemSnapshot, 0) !=
              ERR_SUCCESS)
              printf("-> Unable to rename original snapshot.\n");
        }))
    ...
    if (isMountpoint(systemSnapshotMountPoint)) {
        execute_with_kernel_credentials(^{
          if (unmount(systemSnapshotMountPoint, MNT_FORCE) != ERR_SUCCESS)
              printf("-> Unable to unmount old snapshot mount point.\n");
        });
    }
}
...
void start_rootfs_remount(void) {
    ...
    if (!KERN_POINTER_VALID(rootfs_vnode))
        printf("Unable to get VNode for RootFS\n");
    printf("-> vnode:\t0x%llx\n", rootfs_vnode);
    kptr_t v_mount = kapi_read_kptr(rootfs_vnode + 0xd8);
    printf("-> vmount:\t0x%llx\t\t(0x%llx)\n", v_mount, (rootfs_vnode + 0xd8));
    if (!KERN_POINTER_VALID(v_mount))
        printf("--> Got invalid v_mount pointer!\n");
    const char **snapshots = snapshot_list(rootfd);
    char *systemSnapshot = copySystemSnapshot();
    if (systemSnapshot == NULL)
        printf("--> Unable to copy system snapshot.\n");
    ...
    if (snapshots == NULL) {
        ...
        if (!KERN_POINTER_VALID(dev_v_node))
            printf("---> Unable to get vnode for root device!\n");
        kptr_t v_specinfo = kapi_read64(dev_v_node + 0x78);
        if (!KERN_POINTER_VALID(v_specinfo))
            printf("---> Unable to get specinfo for root device.\n");
        ...
        if ((v_flag & MNT_RDONLY) || (v_flag & MNT_NOSUID)) {
            ...
            if (mount("apfs", "/", MNT_UPDATE, (void *)&opts) != ERR_SUCCESS)
                printf("--> Unable to remount RootFS.\n");
            kapi_write32(v_mount + 0x70, v_flag);
        }
        if (!has_original_snapshot)
            if (oldest_snapshot != NULL)
                printf("-> Trying to rename oldest snapshot...\n");
        close(rootfd);
        ...
        if (isMountpoint("/var/MobileSoftwareUpdate/mnt1"))
        ...
        if (deleteFile(rootFsMountPoint) != true)
            printf("-> Unable to clean old rootfs mount point\n");
        if (ensureDirectory(rootFsMountPoint, getuid(), 0755) != true) {
            printf("-> Unable to create mount point. (Missing parent "
                   "directory)\n");
        }
        const char *argv[] = {"/sbin/mount_apfs", thedisk, rootFsMountPoint,
                              NULL};
        if (runCommandv(
                argv[0], 3, argv,
                ^(pid_t pid) {
                  kptr_t procStructAddr = kproc_find_by_pid(pid);
                  if (KERN_POINTER_INVALID(procStructAddr))
                      printf("Unable to find mount_apfs's process in kernel "
                             "memory. "
                             "(%s/%d)\n",
                             get_path_for_pid(pid), pid);
                  give_creds_to_proc_at_addr(
                      procStructAddr, get_kernel_cred_addr(g_exp.kernel_proc));
                },
                true, false) != ERR_SUCCESS)
            printf("-> Unable to mount RootFS.\n");
        if (util_runCommand("/sbin/mount", NULL) != ERR_SUCCESS)
            printf("-> Unable to print new mount list.\n");
        const char *systemSnapshotLaunchdPath =
            [@(rootFsMountPoint) stringByAppendingPathComponent:@"sbin/launchd"]
                .UTF8String;
        if (waitForFile(systemSnapshotLaunchdPath) != ERR_SUCCESS)
            printf("-> Unable to verify newly mounted RootFS.\n");
        printf("-> Successfully mounted RootFS.\n");
        printf("-> Renaming system snapshot...\n");
        close(rootfd);
        rootfd = open(rootFsMountPoint, O_RDONLY);
        if (rootfd <= 0)
            printf("Unable to open newly mounted RootFS.\n");
        rootfs_vnode = find_vnode_with_fd(g_exp.self_proc, rootfd);
        if (!KERN_POINTER_VALID(rootfs_vnode))
            printf("-> Unable to get vnode for newly mounted RootFS.\n");
        v_mount = kapi_read_kptr(rootfs_vnode + 0xd8);
        if (!KERN_POINTER_VALID(v_mount))
            printf("-> Unable to get mount info for newly mounted RootFS.\n");
        snapshots = snapshot_list(rootfd);
        if (snapshots == NULL)
            printf("-> Unable to get snapshots for newly mounted RootFS.\n");
        ...
        for (const char **snapshot = snapshots; *snapshot; snapshot++) {
            ...
            if (strcmp(*snapshot, original_snapshot) == 0) {
                ...
                if (!execute_with_kernel_credentials(^{
                    ...
                    printf(
                        "--> Unable to clear old original system snapshot.\n");
            }
        }
    ...
    if (util_runCommand("/sbin/mount", NULL) != ERR_SUCCESS)
        printf("-> Unable to print new mount list.\n");
    NSString *file = [NSString stringWithContentsOfFile:@"/.manticore"
    ...
    if (!has_original_snapshot) {
        if (oldest_snapshot != NULL) {
            if (execute_with_kernel_credentials(^{
                  if (fs_snapshot_rename(rootfd, oldest_snapshot,
                                         original_snapshot, 0) != ERR_SUCCESS)
                      printf("-> Unable to rename oldest snapshot!\n");
                }))
                printf("-> Successfully renamed oldest snapshot\n");
        } else if (needStrap) {
            if (execute_with_kernel_credentials(^{
                  if (fs_snapshot_create(rootfd, original_snapshot, 0) !=
                      ERR_SUCCESS)
                      printf("-> Unable to create stock snapshot!\n");
                }))
...
bool check_root_write(void) {
    [[NSFileManager defaultManager] createFileAtPath:@"/.manticore_rw"
                                            contents:nil
                                          attributes:nil];
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/.manticore_rw"]) {
        [[NSFileManager defaultManager] removeItemAtPath:@"/.manticore_rw"
                                                   error:nil];
        return true;
    }
    return false;
...
bool check_root_read(void) {
    NSString *systemVersionPlist =
        @"/System/Library/CoreServices/SystemVersion.plist";
    NSDictionary *snapshotSystemVersion =
        [NSDictionary dictionaryWithContentsOfFile:systemVersionPlist];
    if (snapshotSystemVersion[@"ProductVersion"])
        return true;
    if ([snapshotSystemVersion count] > 0)
        return true;
    return false;
}
