...
bool set_platform_binary(kptr_t proc, bool set) {
    ...
    if (!KERN_POINTER_VALID(proc))
        return 0; // what the fuck? proc needs to be invalid?
...
kptr_t give_creds_to_proc_at_addr(kptr_t proc, kptr_t creds) {
    ...
    auto old_creds =
        kapi_read_kptr(our_creds); // store them for restoration later

    if (KERN_POINTER_INVALID(old_creds)) {
        manticoreWarn(
            "[give_creds_to_proc_at_addr] old_creds invalid value: %#0llx",
            old_creds);
        return (kptr_t)NULL;
    } else
        manticoreInfo("[give_creds_to_proc_at_addr] old_creds stored at %#0llx",
                      old_creds);
...
bool execute_with_credentials(kptr_t proc, kptr_t creds,
                              void (^function)(void)) {
    MANTICORE_THROW_ON_FALSE(KERN_POINTER_VALID(proc));
    MANTICORE_THROW_ON_FALSE(KERN_POINTER_VALID(creds));
    ...
    if (KERN_POINTER_INVALID(old_creds)) {
        manticoreWarn(
            "[execute_with_credentials] old_creds invalid value: %#0llx",
            old_creds);
        return false;
    }
...
kptr_t get_kernel_cred_addr() {
    ...
    if (KERN_POINTER_INVALID(k_ucred)) {
        manticoreWarn("[get_kernel_cred_addr] k_ucred invalid value: %#0llx",
                      k_ucred);
        return (kptr_t)NULL;
    } else
        manticoreInfo("[get_kernel_cred_addr] kernel credits found @ 0x%llx",
                      k_ucred);
...
bool execute_with_kernel_credentials(void (^function)(void)) {
    ...
    if (KERN_POINTER_INVALID(k_cred)) {
        manticoreWarn(
            "[execute_with_kernel_credentials] k_cred invalid value: %#0llx",
            k_cred);
        return false;
    }
    ...
    if (!execute_with_credentials(g_exp.self_proc, k_cred, function)) {
        manticoreWarn(
            "[execute_with_kernel_credentials] failed to execute as kernel :(");
        return false;
    } else
        manticoreInfo("[execute_with_kernel_credentials] successfully "
                      "executed as kernel :)");
...
uint64_t proc_of_pid(pid_t pid) {
    ...
    //    while (proc) { //iterate over all processes till we find the one we're
    //    looking for
...
kptr_t find_vnode_with_fd(kptr_t proc, int fd) {
    ...
    if (fd <= 0 || !KERN_POINTER_VALID(proc))
        return 1;
    kptr_t fdp = read_64(proc + 0xf8);
    if (!KERN_POINTER_VALID(fdp))
        return 2;
    kptr_t ofp = read_64(fdp + 0x0);
    if (!KERN_POINTER_VALID(ofp))
        return 3;
    kptr_t fpp = read_64(ofp + (fd * sizeof(kptr_t)));
    if (!KERN_POINTER_VALID(fpp))
        return 4;
    kptr_t fgp = read_64(fpp + 0x10);
    if (!KERN_POINTER_VALID(fgp))
        return 5;
    kptr_t vnode = read_64(fgp + 0x38);
    if (!KERN_POINTER_VALID(vnode))
        return 6;
    ret = vnode;
    return ret;
}
...
kptr_t find_allproc() {
    ...
    while (true) {
        ...
        if (KERN_POINTER_VALID(next_proc))
            current_proc = next_proc;
        if (KERN_POINTER_INVALID(next_proc))
            break;
    }
