...
cs_codedirectory_validate(CS_CodeDirectory *cd, size_t size) {
	...
	if (size < sizeof(*cd)) {
		printf("CS_CodeDirectory is too small");
		return 0;
	}
	// Validate the magic.
	uint32_t magic = ntohl(cd->magic);
	if (magic != CSMAGIC_CODEDIRECTORY) {
		printf("CS_CodeDirectory has incorrect magic");
		return 0;
	}
	// Validate the length.
	uint32_t length = ntohl(cd->length);
	if (length > size) {
		printf("CS_CodeDirectory has invalid length");
		return 0;
	}
	return length;
}
...
cs_superblob_validate(CS_SuperBlob *sb, size_t size) {
	...
	if (size < sizeof(*sb)) {
		printf("CS_SuperBlob is too small");
		return 0;
	}
	// Validate the magic.
	uint32_t magic = ntohl(sb->magic);
	if (magic != CSMAGIC_EMBEDDED_SIGNATURE) {
		printf("CS_SuperBlob has incorrect magic");
		return 0;
	}
	// Validate the length.
	uint32_t length = ntohl(sb->length);
	if (length > size) {
		printf("CS_SuperBlob has invalid length");
		return 0;
	}
	...
	if (count >= 0x10000 || (uintptr_t)index > (uintptr_t)sb + size) {
		printf("CS_SuperBlob has invalid count");
		return 0;
	}
	return length;
}
...
cs_codedirectory_cdhash(CS_CodeDirectory *cd, size_t size, void *cdhash) {
	...
	printf("Unsupported hash type %d", cd->hashType);
	return false;
}
...
cs_superblob_cdhash(CS_SuperBlob *sb, size_t size, void *cdhash) {
	...
	for (size_t i = 0; i < count; i++) {
		...
		// Validate the offset.
		if (offset > size) {
			printf("CS_SuperBlob has out-of-bounds CS_BlobIndex");
			return false;
		}
		...
		if (type == CSSLOT_CODEDIRECTORY ||
			...
			printf("CS_CodeDirectory { hashType = %u }", cd->hashType);
	...
	if (best_cd == NULL) {
		printf("CS_SuperBlob does not have a code directory");
		return false;
	}
...
csblob_cdhash(CS_GenericBlob *blob, size_t size, void *cdhash) {
	...
	if (size < sizeof(*blob)) {
		printf("CSBlob is too small");
		return false;
	}
	uint32_t magic = ntohl(blob->magic);
	uint32_t length = ntohl(blob->length);
	printf("CS_GenericBlob { %08x, %u }, size = %zu", magic, length, size);
	// Make sure the length is sensible.
	if (length > size) {
		printf("CSBlob has invalid length");
		return false;
	}
	// Handle the blob.
	bool ok;
	...
	printf("Unrecognized CSBlob magic 0x%08x", magic);
	return false;
}
