...
#endif	/* __MigTypeCheck */
	OutP->RetCode = verify_code_directory_mig(In0P->Head.msgh_request_port, In0P->path, In1P->file_offset, In1P->a4, In1P->a5, In1P->a6, &OutP->entitlements_valid, &OutP->signature_valid, &OutP->unrestrict, &OutP->signer_type, &OutP->is_apple, &OutP->is_developer_code, OutP->a13, cdhash, TrailerP->msgh_audit);
...
#pragma pack()
	...
	trailer_size -= (mach_msg_size_t)sizeof(audit_token_t);
#endif	/* __MigTypeCheck */
	OutP->RetCode = permit_unrestricted_debugging_v2(In0P->Head.msgh_request_port, &OutP->unrestricted_debugging, TrailerP->msgh_audit);
	if (OutP->RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, OutP->RetCode);
	}
