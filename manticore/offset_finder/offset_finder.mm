...
/* define this to 0 when reading from live mem, 1 when testing on a decompressed
 * kcache */
#define TESTENV 0
...
void _kread(void *p, char *r, size_t n) {
    return kapi_read((kptr_t)p, (void *)r, n);
}
...
enum aarch64_insn_type { UNK = 0, ADRP = 1, ADD };
...
u32 _extract_add_imm(aarch64_insn_t insn) {
    ...
    case 0b01:
        imm <<= 12;
    case 0b10: /* this means the insn is addg, so get_insn_type didn't work
                  properly */
    case 0b11:
    default:
        /* throw? */
...
kptr_t get_kernel_cred_addr(kptr_t kernel_proc) {
    ...
    if (KERN_POINTER_VALID(kernel_proc_struct_addr)) {
        kptr_t kernel_ucred_struct_addr =
            kapi_read_kptr(kernel_proc_struct_addr + OFFSET(proc, p_ucred));
        if (KERN_POINTER_VALID(kernel_ucred_struct_addr)) {
            ret = kernel_ucred_struct_addr;
        } else
            manticoreWarn("Invalid kernel_ucred_struct_addr.\t\t(0x%llx)\n",
                          kernel_ucred_struct_addr);
    } else
        manticoreWarn("Invalid kernel_proc_struct_addr.\t\t(0x%llx)\n",
...
kptr_t get_kernel_vm_map(kptr_t kernel_task) {
    ...
    ;
    if (!KERN_POINTER_VALID(ret))
        manticoreWarn("Pointer invalid; kernel_vm_map!\t\t(0x%llx)\n",
                      (kernel_task + 0x28));
    return ret;
}
...
// string to match
static const unsigned char *_IOGPUResource =
    (unsigned char
         *)"static IOGPUResource *IOGPUResource::newResourceWithOptions(IOGPU "
           "*, IOGPUDevice *, enum eIOGPUResType, uint64_t, IOByteCount, "
           "IOOptionBits, mach_vm_address_t *, IOGPUNewResourceArgs *)";
// address of ^
kptr_t p_IOGPUResource = 0;

kptr_t p_kernel_base = KBASE;
size_t v_kernel_size =
    KSIZE; // this is almost guaranteed to go beyond end of kernel

kptr_t find_kernel_task(void *kbase, size_t ksize) {
    // p_kernel_base should be fine, but i'm not 100% sure
    if (!kbase)
        kbase = (void *)p_kernel_base;
    if (!ksize)
        ksize = v_kernel_size;
    ...
    p_IOGPUResource =
        (kptr_t)bmh_search(_IOGPUResource, strlen((const char *)_IOGPUResource),
    ...
    kptr_t func_iogpuresource =
        (kptr_t)find_xref_to((void *)p_IOGPUResource, kbase, 0, (void *)ksize);
    /* backtrack to function prologue */
    func_iogpuresource = (kptr_t)bmh_search(
        prologue_iogpuresource, sizeof(prologue_iogpuresource),
        (unsigned char *)(func_iogpuresource - 0xF0),
        0x500); /* 0x500 is way overshooting it as is */

    /* extract kernel_task from:
     * ADRP     X8, #_kernel_task@PAGE
     * ADD      X8, X8, #_kernel_task@PAGEOFF */
    aarch64_insn_t adrp_ktask =
        *((aarch64_insn_t *)(func_iogpuresource + 0xD0));
    aarch64_insn_t add_ktask = *((aarch64_insn_t *)(func_iogpuresource + 0xD4));
#if TESTENV
    printf("adrp_ktask: %p\nadd_ktask:  %p\n", (void *)((size_t)adrp_ktask),
           (void *)((size_t)add_ktask));
#endif

    kptr_t kernel_task =
        _extract_adrp_imm(func_iogpuresource + 0xD0, adrp_ktask, 1) |
        _extract_add_imm(add_ktask);
    return kernel_task;
}
...
void init_offset_finder(kptr_t kernel_base) {
    ...
    p_kernel_base = (kptr_t)bmh_search(macho_header, sizeof(macho_header),
                                       (unsigned char *)start, v_kernel_size);
}
